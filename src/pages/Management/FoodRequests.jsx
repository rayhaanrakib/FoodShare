...
import FoodRequestsList from '../Management/FoodRequestsList';
...
const FoodRequests = () => {
    ...
    const url = `https://foodshare-server-mu.vercel.app/user/requested?email=${user?.email}`;
    useEffect(() => {
        fetch(url)
            .then(res => res.json())
            .then(data => {
    ...
    return (
        <div className='container mx-auto px-5'>
            <Helmet><title>FoodConnect | Requested Foods</title></Helmet>
                        ...
                        (
                            <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                                ...
                                <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pt-10 pb-16 text-center">
                                    <h1 class="mx-auto max-w-md md:max-w-lg font-display text-3xl font-medium tracking-tight text-slate-900 lg:text-5xl">
                                    ...
                                    <p class="mx-auto mt-5 max-w-2xl text-lg tracking-tight text-slate-500 sm:mt-6">
                                ...
                                <table class="w-full text-sm text-left text-gray-500">
                                    ...
                                    <tbody>
                                        {
                                            requestedFoods.map(requested => <FoodRequestsList key={requested._id} requested={requested}></FoodRequestsList>)
                                        }
                                    </tbody>
                                </table>
                                {
                                    requestedFoods.length === 0 ? (<div><h2 className='text-center font-bold text-accent text-xl my-5'>No Requests Yet</h2></div>) : (<div></div>)
                                }
                            </div>
                        )
                }
