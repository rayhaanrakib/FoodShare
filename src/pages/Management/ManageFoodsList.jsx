...
const handleDelete = id => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            fetch(`https://foodshare-server-mu.vercel.app/user/services/${id}`, {
                method: 'DELETE'
            })
                .then(res => res.json())
                .then(data => {
    ...
    columnHelper.accessor('action', {
        ...
        cell: ({ row }) => (
            <Stack justifyContent={'center'} direction={'row'} spacing={1}>
                <IconButton component={Link} to={`requests/${row.original._id}`}>
                    <ManageSearchIcon />
                </IconButton>
                <IconButton
                    component={Link}
...
const ManageFoodsList = ({ manageFoods }) => {
...
export default ManageFoodsList;
